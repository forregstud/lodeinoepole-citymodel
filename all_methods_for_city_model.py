# -*- coding: utf-8 -*-
"""all methods for city model.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1a9EsjnzfkBlNuV2PZtPyPlkgulcoy1oX
...
boundary = gpd.read_file('/content/Границы ЛП + проект (их в модель).geojson')
...
#water = roads[roads.geom_type.isin(['Polygon', 'MultiPolygon'])]
...
blocks.to_file('blocks_2.geojson')

"""Граф, доступность, связанность"""
...
"""Обеспеченность сервисами (боже поможи)"""

import warnings
warnings.filterwarnings("ignore")

buildings = gpd.read_file("/content/building_2.geojson")
...
buildings.to_file("building(2).geojson")
...
directory = os.fsencode("Гипотеза 2/Сервисы 2/")
for file in os.listdir(directory):
    filename = os.fsdecode(file)
    if filename.endswith(".geojson"):
        service_name = filename.removesuffix(".geojson")
        print(f"Adding service {service_name}")
        service_gdf = gpd.read_file("Гипотеза 2/Сервисы 2/" + filename)
...
prov.plot(prov_res)
city.to_pickle("lp_bn_model_2.pkl")
...
LP = city.from_pickle('lp_bn_model_2.pkl') # считываем в переменную готовую модель
print(LP)
...
diversity = Diversity(city_model=LP)
...
centrality = Centrality(city_model=LP)
...
centrality_population = PopulationCentrality(city_model=LP)
...
spacematrix = Spacematrix(city_model=LP)
...
lup = LandUsePrediction(city_model=LP)
